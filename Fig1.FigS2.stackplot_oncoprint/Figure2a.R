...
outfile=paste0("Output.Manuscript/Figure2a.v",format(Sys.time(), "%Y%m%d"),".pdf") 

####################################################
mfdata <- read.table(molfeatfile, header=T, sep='\t');
mfdata = mfdata[ mfdata$icluster%in%c("IC1","IC2","IC3","IC4","IC5"),]
...
sel.col=c("proliferative_index","cyt_score" ,"hrd_index","tmb","tmb_norm","tumor_purity_pathologist")
...
ht_opt("legend_labels_gp" = gpar(fontsize = 17),legend_grid_height=unit(6, "mm"))
...
columnha <-HeatmapAnnotation(icluster = Matrix.data9$icluster,
                             ...
                             col = list(icluster =c("IC1"="#E41A1C","IC2"="#377EB8","IC3"="#4DAF4A","IC4"="#984EA3", "IC5"="gray"),
                             ...
                             annotation_height = unit(c(4,4,4,4 ), "mm"),
...
ht_list <-Heatmap(as.matrix(t(Matrix.data1)),  column_split=mfdata[,'icluster'], column_title_gp = gpar(fontsize = 25,fontface= "bold"), column_dend_height = unit(2, "cm"),
  ...
  HeatmapAnnotation(	"Tumor Purity" = anno_barplot(Matrix.data7$tumor_purity_pathologist, ylim = c(0, max(Matrix.data7$tumor_purity_pathologist, na.rm = TRUE)), axis = TRUE,axis_param = list(side = "left")),
  ...
  HeatmapAnnotation( "Paloma3 factors" = anno_barplot(as.matrix(  (Matrix.data5) ), show_legend=TRUE, 
                                                      gp = gpar(fill = c("green", "yellow","red", "blue","magenta","gray"),col= c("green", "yellow","red", "blue","magenta","gray")),bar_width = 1),
                     annotation_name_gp= gpar(fontsize = 18),height = unit(2, "cm"))%v%
  Heatmap(as.matrix(t(Matrix.data4.z)),row_dend_side = "left",row_names_side ="right",  heatmap_height = unit(6, "mm")*nrow(t(Matrix.data4.z)),
          show_column_names = FALSE,cluster_columns = TRUE,cluster_column_slices = FALSE,name="ESTIMATE z-score",
          heatmap_legend_param = list( labels_gp  = gpar(fontsize = 15),grid_height=unit(5, "mm"),at = c(-4, -2, 0, 2, 3.9 ),labels = c(-4, -2, 0, 2, 4) )) %v%
  Heatmap(as.matrix(t(Matrix.data3.z )),row_dend_side = "left",row_names_side ="right", heatmap_height = unit(6, "mm")*nrow(t(Matrix.data3.z)),
          show_column_names = FALSE,cluster_columns = TRUE,cluster_column_slices = FALSE,name="PAM50 z-score",
          heatmap_legend_param = list( labels_gp  = gpar(fontsize = 15),grid_height=unit(5, "mm"),at = c(-4, -2, 0, 2, 3.9 ),labels = c(-4, -2, 0, 2, 4) )) %v%
  Heatmap(as.matrix(t(Matrix.data2)),row_dend_side = "left",row_names_side ="right",   heatmap_height = unit(6, "mm")*nrow(t(Matrix.data2)),
          show_column_names = FALSE,cluster_columns = TRUE,cluster_column_slices = FALSE,name="GSVA",
          heatmap_legend_param = list( labels_gp  = gpar(fontsize = 15),grid_height=unit(5, "mm"),at = c(-1, -0.5, 0, 0.5, 0.9 ),labels = c(-1, -0.5, 0, 0.5, 1) )) %v%
  Heatmap(as.matrix(t(Matrix.data1)),row_dend_side = "left",row_names_side ="right",column_split=mfdata[,'icluster'],  heatmap_height = unit(6, "mm")*nrow(t(Matrix.data1)),
          show_column_names = FALSE,  cluster_columns = TRUE,cluster_column_slices = FALSE,name="EXPR",
          heatmap_legend_param = list( labels_gp  = gpar(fontsize = 15),grid_height=unit(5, "mm"),at = c(0, 2 ,4,6,7.9 ),labels = c(0,2,4,6,8) ))  

lgd_Matrix.data5 = Legend(labels = colnames(Matrix.data5), title = "Paloma3 factors", 
                          title_gp = gpar(fontsize = 18),labels_gp  = gpar(fontsize = 17),grid_height=unit(6, "mm"),
                          legend_gp = gpar( fill = c("green", "yellow","red", "blue","magenta","gray")  ) )


#dev.off()
pdf(  outfile ,24,12,useDingbats=F)
draw(ht_list, merge_legend = TRUE ,padding = unit(c(2, 2, 2, 16), "mm"),
