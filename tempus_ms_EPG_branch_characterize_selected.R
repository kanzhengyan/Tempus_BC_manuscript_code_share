...
# input/output files
plotfile <- paste(outdir, 'tempus_ms_branch_characterize_selected.pdf', sep='/');
...
mdata <- mdata[mdata$branch %in% c('A', 'B', 'C', 'D'),];
colnames(mdata) <- gsub('^h.HALLMARK', 'HALLMARK', colnames(mdata));

# specifiy color scheme
colors_branch = brewer.pal(4, 'Set1')[c(4,3,2,1)]
...
foo$branch <- factor(foo$branch, levels = c('A', 'B', 'C', 'D'));
...
HR_CI_label3 <- paste('(', round(exp(confint(model))['branchD', '2.5 %' ], 2), '-', 
                      round(exp(confint(model))['branchD', '97.5 %' ], 2), ')', sep='')
...
label3 <- paste("D: HR=", format(coef(summary(model))["branchD", "exp(coef)"], digits = 4), 
                ", p=", format(coef(summary(model))["branchD", "Pr(>|z|)"], digits = 4, sci = TRUE), sep = "");
...
plist = alist();
comps = list(c('A', 'B'), c('A', 'C'), c('C', 'D'), c('A', 'D'));
orders = c('A', 'B', 'C', 'D');
for (i in 1:3){
