...
plotfile <- paste(outdir, 'Fig-4a.pseudotime_covariate_heatmap.pdf', sep='/');
...
mm <- mfdata[!is.na(mfdata$branch_epg20),];
...
gsvadata <- t(gsvadata[mm$sample_id,-1]);
...
cc <- cor(t(rbind(gsvadata, mm$pseudotime)));
ccvals <- cc[51,1:50]
pvals <- apply(gsvadata, 1, function(x) {
  res <- cor.test(x, mm$pseudotime);
  return(res$p.value)
})
rvals <- apply(gsvadata, 1, function(x) {
...
gsva_cor <- gsvadata[rownames(gsvadata) %in% genesets_cor,];
print(res[res$genset <- "ESTROGEN_RESPONSE_EARLY",])
...
pdf(file=plotfile, onefile=T, height=15, width=34, pointsize=12);
...
annot_colors = list(
  ...
  icluster = c('IC1'=colors_icluster[1], 'IC2'=colors_icluster[2], 'IC3'=colors_icluster[3], 'IC4'=colors_icluster[4], 'IC5'=colors_icluster[5]),
...
draw(ht_list, heatmap_legend_side="left", annotation_legend_side="left",
     legend_grouping = "original", legend_labels_gp = gpar(fontsize = 20))
